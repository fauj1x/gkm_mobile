...
import 'package:gkm_mobile/models/luaranmhs_hkipatenmhs.dart'; // Pastikan path sesuai
import 'package:gkm_mobile/services/api_services.dart';

class HkiPatenMhsScreen extends StatefulWidget {
  final dynamic tahunAjaran; // Jika diperlukan
  const HkiPatenMhsScreen({Key? key, this.tahunAjaran}) : super(key: key);

  @override
  _HkiPatenMhsScreenState createState() => _HkiPatenMhsScreenState();
}

class _HkiPatenMhsScreenState extends State<HkiPatenMhsScreen> {
  List<HkiPatenMhs> dataList = [];
  ApiService apiService = ApiService();

  String menuName = "Haki & Paten Mahasiswa";
  String endPoint = "hki-paten-mhs"; // Sesuaikan endpoint API kamu
  int userId = 0;
  ...
  Future<void> _fetchData() async {
    try {
      final data = await apiService.getData(HkiPatenMhs.fromJson, endPoint);
      setState(() {
        dataList = data;
      });
    } catch (e) {
  ...
  Future<void> _addData(Map<String, dynamic> newData) async {
    try {
      await apiService.postData(HkiPatenMhs.fromJson, newData, endPoint);
      _fetchData();
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _editData(int id, Map<String, dynamic> updatedData) async {
    try {
      await apiService.updateData(HkiPatenMhs.fromJson, id, updatedData, endPoint);
      _fetchData();
      ScaffoldMessenger.of(context).showSnackBar(
              ...
              children: [
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      _headerCell("No", 50),
                      _headerCell("Judul Penelitian", 200),
  ...
  void _showAddDialog() {
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        title: const Text('Tambah Haki & Paten'),
        content: SingleChildScrollView(
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              TextField(controller: judulController, decoration: const InputDecoration(labelText: 'Judul Penelitian')),
  ...
  void _showEditDialog(HkiPatenMhs data) {
    ...
    showDialog(
      context: context,
      builder: (_) => AlertDialog(
        title: const Text('Edit Haki & Paten'),
        content: SingleChildScrollView(
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
