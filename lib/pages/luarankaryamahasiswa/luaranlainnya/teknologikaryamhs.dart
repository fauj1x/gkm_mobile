// lib/widgets/teknologi_karya_mahasiswa_widget.dart
import 'package:flutter/material.dart';
import 'package:gkm_mobile/models/luaranmhs_teknologimhs.dart'; // Sesuaikan path ini
import 'package:gkm_mobile/models/tahun_ajaran.dart'; // Jika masih relevan, pertahankan
import 'package:gkm_mobile/services/api_services.dart';
import 'package:shared_preferences/shared_preferences.dart';

class TeknologiKaryaMahasiswa extends StatefulWidget {
  final TahunAjaran? tahunAjaran; // Dapat dihapus jika tidak digunakan
  const TeknologiKaryaMahasiswa({Key? key, this.tahunAjaran}) : super(key: key);

  @override
  TeknologiKaryaMahasiswaState createState() => TeknologiKaryaMahasiswaState();
}

class TeknologiKaryaMahasiswaState extends State<TeknologiKaryaMahasiswa> {
  List<TeknologiKaryaMahasiswaModel> dataList = [];
  ApiService apiService = ApiService();
  String menuName = "Data Teknologi Karya Mahasiswa";
  String subMenuName = ""; // Biarkan kosong jika tidak ada sub-menu
  String endPoint = "luaran-teknologikarya-mahasiswa"; // Endpoint API
  int userId = 0;
  ...
  Future<void> _addData(Map<String, dynamic> newData) async {
    try {
      await apiService.postData(TeknologiKaryaMahasiswaModel.fromJson, newData, endPoint);
  ...
  Future<void> _editData(int id, Map<String, dynamic> updatedData) async {
    try {
      await apiService.updateData(TeknologiKaryaMahasiswaModel.fromJson, id, updatedData, endPoint);
                        ...
                        children: [
                          ...
                          Container(
                            ...
                            child: Row(
                              children: [
                                _headerCell("No.", 50),
                                _headerCell("Luaran Penelitian", 200),
                              ...
                              return TableRow(
                                children: [
                                  ...
                                  TableCell(
                                    child: Padding(
                                      padding: const EdgeInsets.all(8.0),
                                      child: Text(data.luaranPenelitian),
                                    ),
                                  ),
                                  TableCell(
                                    child: Padding(
  ...
  void _showAddDialog() {
    ...
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          title: const Text('Tambah Data Teknologi Karya Mahasiswa'),
          content: SingleChildScrollView(
            child: Column(
              mainAxisSize: MainAxisSize.min,
              children: [
                TextField(
                  controller: luaranPenelitianController,
                  decoration: const InputDecoration(labelText: 'Luaran Penelitian'),
                ),
                TextField(
                ...
                TextField(
                  ...
                  maxLines: 3, // Multi-line input for description
                ),
              ],
            ),
          ),
          actions: [
            ...
            TextButton(
              onPressed: () {
                // Validasi input
                if (luaranPenelitianController.text.isEmpty ||
  ...
  void _showEditDialog(int id, TeknologiKaryaMahasiswaModel currentData) {
    final TextEditingController luaranPenelitianController = TextEditingController(text: currentData.luaranPenelitian);
    ...
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          title: const Text('Edit Data Teknologi Karya Mahasiswa'),
          content: SingleChildScrollView(
            child: Column(
              mainAxisSize: MainAxisSize.min,
              children: [
