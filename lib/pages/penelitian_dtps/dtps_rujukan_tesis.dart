...
import 'package:gkm_mobile/models/dtps_aio.dart';
import 'package:gkm_mobile/models/tahun_ajaran.dart';
import 'package:gkm_mobile/services/api_services.dart';
import 'package:shared_preferences/shared_preferences.dart';

class DtpsRujukanTesis extends StatefulWidget {
  final TahunAjaran tahunAjaran;
  const DtpsRujukanTesis({Key? key, required this.tahunAjaran}) : super(key: key);
  @override
  DtpsRujukanTesisState createState() => DtpsRujukanTesisState();
}

class DtpsRujukanTesisState extends State<DtpsRujukanTesis> {
  // final List<String> yesNoOptions = ['Tidak', 'Ya'];
  List<DtpsAioModel> dataList = [];
  ApiService apiService = ApiService();
  String menuName = "Penelitian DTPS";
  String subMenuName = "yang Menjadi Rujukan Tema Tesis/Disertasi";
  String endPoint = "rujukan-tesis";
  int userId = 0;
  ...
  Future<void> _addData(Map<String, dynamic> newData) async {
    try {
      await apiService.postData(DtpsAioModel.fromJson, newData, endPoint);
      _fetchData();
    } catch (e) {
  ...
  Future<void> _editData(int index, Map<String, dynamic> updatedData) async {
    try {
      await apiService.updateData(
          DtpsAioModel.fromJson, index, updatedData, endPoint);
      _fetchData();
    } catch (e) {
                          ...
                          Container(
                            ...
                            child: Row(
                              children: [
                                ...
                                _headerCell("Tema Penelitian sesuai Roadmap", 100),
                                _headerCell("Nama Mahasiswa", 100),
                                _headerCell("Judul Tesis/Disertasi", 100),
                          ...
                          Table(
                            ...
                            children: dataList.asMap().entries.map((entry) {
                              List<String> row = [
                                ...
                                entry.value.temaPenelitian,
                                entry.value.namaMahasiswa,
                                entry.value.judul,
                                entry.value.tahun,
                              ];
                              return TableRow(
            ...
            child: Column(
              children: [
                ...
                TextField(
                  ...
                  decoration: const InputDecoration(labelText: "Judul Tesis/Disertasi"),
                ),
                TextField(
            ...
            TextButton(
              onPressed: () {
                _addData({
                  ...
                  'tema_penelitian': controller2.text,
                  'nama_mahasiswa': controller3.text,
                  'judul': controller4.text,
  ...
  void _showEditDialog(int id, Map<String, dynamic> currentData) {
    final TextEditingController controller1 = TextEditingController(
        text: currentData['nama_dosen'] ?? '');
    final TextEditingController controller2 = TextEditingController(
        text: currentData['tema_penelitian'] ?? '');
    final TextEditingController controller3 = TextEditingController(
        text: currentData['nama_mahasiswa'] ?? '');
    final TextEditingController controller4 = TextEditingController(
        text: currentData['judul'] ?? '');
    final TextEditingController controller5 = TextEditingController(
        text: currentData['tahun'] ?? '');
            ...
            child: Column(
              children: [
                ...
                TextField(
                  controller: controller2,
                  decoration:
                  const InputDecoration(labelText: "Tema Penelitian"),
                ),
                TextField(
                  controller: controller3,
                  decoration:
                ...
                TextField(
                  controller: controller4,
                  decoration:
                  const InputDecoration(labelText: "Judul Tesis/Disertasi"),
                ),
                TextField(
                  controller: controller5,
                  decoration:
