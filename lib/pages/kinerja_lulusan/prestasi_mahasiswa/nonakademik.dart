...
import 'package:gkm_mobile/models/kinerjalulusan_nonakademik.dart'; // Sesuaikan path ini jika perlu
...
class PrestasiNonAkademik extends StatefulWidget {
  ...
  const PrestasiNonAkademik({Key? key, this.tahunAjaran}) : super(key: key);

  @override
  PrestasiNonAkademikState createState() => PrestasiNonAkademikState();
}

class PrestasiNonAkademikState extends State<PrestasiNonAkademik> {
  List<PrestasiNonAkademikModel> dataList = [];
  ApiService apiService = ApiService();
  String menuName = "Data Prestasi Non-Akademik";
  String subMenuName = ""; // Biarkan kosong jika tidak ada sub-menu
  String endPoint = "prestasi-non-akademik"; // Endpoint API untuk Prestasi Non-Akademik
  int userId = 0;
  ...
  Future<void> _fetchData() async {
    try {
      final data = await apiService.getData(PrestasiNonAkademikModel.fromJson, endPoint);
      setState(() {
        dataList = data;
      });
    } catch (e) {
  ...
  Future<void> _addData(Map<String, dynamic> newData) async {
    try {
      await apiService.postData(PrestasiNonAkademikModel.fromJson, newData, endPoint);
  ...
  Future<void> _editData(int id, Map<String, dynamic> updatedData) async {
    try {
      await apiService.updateData(PrestasiNonAkademikModel.fromJson, id, updatedData, endPoint);
  ...
  void _showAddDialog() {
    ...
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          title: const Text('Tambah Data Prestasi Non-Akademik'),
            ...
            child: Column(
              ...
              children: [
                ...
                TextField(
                  ...
                  decoration: const InputDecoration(labelText: 'Tingkat (contoh: Provinsi, Nasional)'),
                ),
                TextField(
                  controller: prestasiController,
                  decoration: const InputDecoration(labelText: 'Prestasi (contoh: Juara 1, Medali Emas)'),
                ),
                TextField(
  ...
  void _showEditDialog(int id, PrestasiNonAkademikModel currentData) {
    ...
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          title: const Text('Edit Data Prestasi Non-Akademik'),
          content: SingleChildScrollView(
            child: Column(
              mainAxisSize: MainAxisSize.min,
              children: [
