/// Mapping judul kategori ke nama-nama tabel yang relevan (dari API `rekap`)
final Map<String, List<String>> kategoriTabel = {
  "Kerjasama Tridharma": [
    "Tabel 1.1 Kerjasama Tridharma - Pendidikan",
    "Tabel 1.2 Kerjasama Tridharma - Penelitian",
    "Tabel 1.3 Kerjasama Tridharma - Pengabdian kepada Masyarakat",
  ],
  "Data Mahasiswa": [
    "Tabel 2.a Seleksi Mahasiswa",
    "Tabel 2.b Mahasiswa Asing",
  ],
  "Data Dosen": [
    "Tabel 3.a.1) Dosen Tetap Perguruan Tinggi",
    "Tabel 3.a.2) Dosen Pembimbing Utama Tugas Akhir",
    "Tabel 3.a.3) Ekuivalen Waktu Mengajar Penuh (EWMP) Dosen Tetap Perguruan Tinggi",
    "Tabel 3.a.4) Dosen Tidak Tetap",
    "Tabel 3.a.5) Dosen Industri/Praktisi",
  ],
  "Kinerja Dosen": [
    "Tabel 3.b.1) Pengakuan/Rekognisi Dosen",
    "Tabel 3.b.2) Penelitian DTPS",
    "Tabel 3.b.3) PkM DTPS",
    "Tabel 3.b.4) Pagelaran/Pameran/Presentasi/Publikasi Ilmiah DTPS",
    "Tabel 3.b.5) Karya Ilmiah DTPS yang Disitasi",
    "Tabel 3.b.6) Produk/Jasa DTPS yang Diadopsi oleh Industri/Masyarakat"
  ],
  "Luaran Lainnya": [
    "Tabel 3.b.7) Luaran Penelitian/PkM Lainnya - HKI (Paten, Paten Sederhana)",
    "Tabel 3.b.8) Luaran Penelitian/PkM Lainnya - HKI (Hak Cipta, Desain Produk Industri, dll.)",
    "Tabel 3.b.9) Luaran Penelitian/PkM Lainnya - Teknologi Tepat Guna, Produk, Karya Seni, Rekayasa Sosial",
    "Tabel 3.b.10) Luaran Penelitian/PkM Lainnya - Buku ber-ISBN, Book Chapter",
  ],
  "Kualitas Pembelajaran": [
    "Tabel 5.a Kurikulum, Capaian Pembelajaran, dan Rencana Pembelajaran",
    "Tabel 5.b Integrasi Kegiatan Penelitian/PkM dalam Pembelajaran",
    "Tabel 5.c Kepuasan Mahasiswa",
  ],
  "Penelitian DTPS": [
    "Tabel 6.a Penelitian DTPS + Mahasiswa",
    "Tabel 6.b Penelitian Rujukan Tesis",
  ],
  "PKM DTPS": [
    "Tabel 7 PkM DTPS + Mahasiswa",
  ],
  "IPK Lulusan": [
    "Tabel 8.a IPK Lulusan",
  ],
  "Prestasi Mahasiswa": [
    "Tabel 8.b.1) Prestasi Akademik",
    "Tabel 8.b.2) Prestasi Non-akademik",
  ],
  "Masa Studi Lulusan": [
    "Tabel 8.c Masa Studi Lulusan",
  ],
  "Evaluasi Lulusan": [
    "Tabel 8.d.1) Waktu Tunggu Lulusan",
    "Tabel 8.d.2) Kesesuaian Bidang Kerja Lulusan",
    "Tabel 8.e.1) Tempat Kerja Lulusan",
    "Tabel 8.e.2) Kepuasan Pengguna Lulusan",
  ],
  "Luaran Karya Mahasiswa": [
    "Tabel 8.f.1) Publikasi Mahasiswa",
    "Tabel 8.f.2) Sitasi Mahasiswa",
    "Tabel 8.f.3) Produk Mahasiswa",
    "Tabel 8.f.4) HKI Mahasiswa - Paten",
    "Tabel 8.f.5) HKI Mahasiswa - Hak Cipta",
    "Tabel 8.f.6) Teknologi Mahasiswa",
    "Tabel 8.f.7) Buku Mahasiswa",
  ],
};
